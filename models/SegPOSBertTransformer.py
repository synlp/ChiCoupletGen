...
class SegPOSBertTransformer(nn.Module):

    def __init__(self, directory, tokenizer, bert=None, n_layers=3, 
                 head=4, d_pos=32,
                 d_ff=512, dropout=0.2, lr=0.001,
                 max_len=30, ratio=0.1):
        super(SegPOSBertTransformer, self).__init__()
        assert directory != '' or bert != None, "path and bert cannot be empty at the same time."
        if directory:
            self.model = torch.load(directory + '/model.pt')
            self.clf = torch.load(directory + '/clf.pt')
        else:
            self.model = make_seg_pos_bertmodel(bert=bert,
                                        N=n_layers,
                                        d_ff=d_ff,
                                        h=head,
                                        dropout=dropout,
        ...
        self.model.cuda()
        self.clf.cuda()
        self.name = 'SegPOSBertTransformer'
        self.freeze_encoder()
    ...
    def fit_batch(self, batch) -> float:
        ...
        decoder_output = self.model(src, decoder_input,
                                    src_mask, tar_mask)
        pos_info = self.model.pos_embed(batch['src_pos'])
        generator_input = torch.cat([decoder_output, pos_info], dim=-1)
    ...
    def fit_batch_seg(self, batch) -> float:
        ...
        tar_mask = tar_mask & \
            subsequent_mask(decoder_input.size(-1)).type_as(tar_mask.data)
        decoder_output, hidden = self.model(src, decoder_input,
                                    src_mask, tar_mask, return_hidden=True)
    ...
    def decode_batch(self, src: torch.Tensor, src_mask: torch.Tensor, src_pos: torch.Tensor) -> List[str]:
        batch_size = src.size(0)
        decoder_input = torch.LongTensor([self.tokenizer.cls_token_id]*batch_size).cuda()
        decoder_input = decoder_input.unsqueeze(1)
        pos_info = self.model.pos_embed(src_pos)
        for i in range(src.size(1)-1):
            ...
            decoder_output = self.model(src, 
                                        ...
                                        tar_mask)
            try:
                generator_input = torch.cat([decoder_output, pos_info[:, :i+1, :]], dim=-1)
            except:
                import pdb; pdb.set_trace()
    ...
    def evaluate(self, dataset):
        ...
        # import pdb; pdb.set_trace()
        for batch in pbar:
            decoded_output += self.decode_batch(batch['src'], batch['src_mask'], batch['src_pos'])
            text = batch['tar_text']
    ...
    def generate_response(self, src: str, wait=10) -> str:
        ...
        while not valid:
            ...
            if decoder_input[0, j] != self.tokenizer.sep_token_id:
                continue
            decoded_str = self.tokenizer.convert_ids_to_tokens(decoder_input[1:j].numpy().tolist())
        ...
        return self.tokenizer.convert_tokens_to_string(decoded_str), success
