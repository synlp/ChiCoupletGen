...
class MemoryPOSBertTransformer(nn.Module):
    ...
    def __init__(self, path, tokenizer, bert=None, n_layers=3, 
        ...
        self.model.cuda()
        self.name = 'MemoryPOSBertTransformer'
        self.freeze_encoder()
    ...
    def fit_batch(self, batch) -> float:
        ...
        dep_text = batch['pos_text']
        dep_type = batch['pos_type']
    ...
    def decode_batch(self, src: torch.Tensor, src_mask: torch.Tensor, pos_text, pos_type) -> List[str]:
        ...
        for i in range(self.max_len):
            ...
            tar_mask = tar_mask & \
                subsequent_mask(decoder_input.size(-1)).type_as(tar_mask.data)
            output_mask = (pos_text[:, :, 0] != self.tokenizer.pad_token_id)
            memory_mask = (pos_text != self.tokenizer.pad_token_id)
            decoder_output = self.model(src, decoder_input,
                                        src_mask, tar_mask, 
                                        pos_text, pos_type,
                                        memory_mask, output_mask)
