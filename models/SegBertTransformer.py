...
class SegBertTransformer(nn.Module):
    ...
    def __init__(self, directory, tokenizer, bert=None, n_layers=3, 
        ...
        if directory:
            ...
            self.model = make_seg_bertmodel(bert=bert,
                                        N=n_layers,
                                        d_ff=d_ff,
                                        h=head,
                                        dropout=dropout)
    ...
    def decode_batch(self, src: torch.Tensor, src_mask: torch.Tensor) -> List[str]:
