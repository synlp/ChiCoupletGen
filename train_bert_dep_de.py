...
from models.DecoderDepBertTransformer import DecoderDepBertTransformer
...
trn, dev, tst, human, max_len = preprocess_bert_dep(tokenizer, batch_size, 'data')

# with open('./save/lang.txt', 'wb') as f:
#     pickle.dump(lang, f)

mdl = DecoderDepBertTransformer(path, tokenizer, bert, n_layers, 
...
for epoch in range(max_epochs):
    ...
    with torch.no_grad():
        ...
        mdl.save(f'./save/{mdl.name}-{bleu_score}-{format_acc}-{perplex}.pt')
        print('model saved')
    if early_stopping:
